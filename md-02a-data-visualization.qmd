...
##outlier filtering is from chatGPT
#@data types: continent = factor, lifeExp = numerical
...
```{r}
ggplot(gapminder_summary_continent_year, aes(x=year, y=lifeExp, color=continent)) +
  geom_point() +
  geom_line()
```
...
```{r}
ggplot(gapminder_summary_2007, aes(x=continent, y=count)) +
  geom_col()
```
...
#the plots are the same because the col chart used the count mapping on the y-axis, which is what the bar plot implicitly already does
```
...
#geom hist and geom bar is similar because they both show counts of categories. for bar, the categories are actual categorical values; for hist, they are continuous variables

#breaks changes the width of the bars and therefore the shape of the histogram
```
...
#country_colors contains lists of colors matched to a country. adding ? opens the help window to the object.
```
